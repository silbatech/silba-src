...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSLB stake block is less than allowed start height", __func__);

    if (CZPivStake* zSLB = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zSLB->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSLB stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSLB stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSLB->GetValue()));
        if (nChecksum200 != zSLB->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSLB->GetChecksum(), nChecksum200);
    } else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zSLB fails context checks", __func__);
    } else {
        ...
        CPivStake* slbInput = new CPivStake();
        slbInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(slbInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zSLB is checked above in ContextualCheckZerocoinStake()
