...
CzSLBTracker::CzSLBTracker(std::string strWalletFile)
{
...
CzSLBTracker::~CzSLBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSLBTracker::Init()
{
...
bool CzSLBTracker::Archive(CMintMeta& meta)
{
...
bool CzSLBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSLBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSLBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSLBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSLBTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSLBTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSLBTracker::GetSerialHashes()
{
...
CAmount CzSLBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSLBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSLBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSLBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSLBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSLBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSLBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSLBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSLBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSLBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSLBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSLBWallet* zSLBWallet)
{
    bool iszSLBWalletInitialized = (NULL != zSLBWallet);
    CMintMeta meta;
    ...
    if (! iszSLBWalletInitialized)
        zSLBWallet = new CzSLBWallet(strWalletFile);
    meta.isSeedCorrect = zSLBWallet->CheckSeed(dMint);
    if (! iszSLBWalletInitialized)
        delete zSLBWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSLBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSLBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSLBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSLBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSLBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSLBTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSLBWallet* zSLBWallet = new CzSLBWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSLBWallet);
        }
        delete zSLBWallet;
        LogPrint("zero", "%s: added %d dzslb from DB\n", __func__, listDeterministicDB.size());
    }
