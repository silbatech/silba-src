...
CzSLBWallet::CzSLBWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zslb seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSLBSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSLBSeed_deprecated()) {
                    LogPrintf("%s: Updated zSLB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zslb seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zslb wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSLBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSLBCount(nCountLastUsed);
    else if (!walletdb.ReadZSLBCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSLBWallet::Lock()
{
    seedMaster = 0;
}

void CzSLBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSLBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSLB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSLBWallet::LoadMintPoolFromDB()
{
...
void CzSLBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSLBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSLBWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zslbTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSLBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSLB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zslbTracker which also adds to database
    pwalletMain->zslbTracker->Add(dMint, true);
...
void CzSLBWallet::SeedToZSLB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSLBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSLBWallet::GenerateDeterministicZSLB(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSLBWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSLBWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSLBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
