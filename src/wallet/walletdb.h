...
public:
    ...
    bool WriteZSLBSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSLBSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSLBSeed_deprecated(uint256& seed);
    bool EraseZSLBSeed();
    bool EraseZSLBSeed_deprecated();

    bool WriteZSLBCount(const uint32_t& nCount);
    bool ReadZSLBCount(uint32_t& nCount);
