...
// The zSLB block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zSLB is a hash of the serial
...
{
    //Create an output returning the zSLB that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZSLBOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zSLB output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zSLB", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZSLBOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zSLB output", __func__);
...
{
    CzSLBTracker* zslbTracker = pwallet->zslbTracker.get();
    CMintMeta meta;
    if (!zslbTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zslbTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a SLB stake is the outpoint
