...
#include "ui_zslbcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zslbTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zslbTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSLB.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSLB seed used to mint this zSLB is not the same as currently hold in the wallet";
            else
