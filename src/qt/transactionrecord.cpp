...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZSLBModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zslbTracker->HasMintTx(hash))) {
            //zSLB stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZSLB;
...
bool IsZSLBType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZSLB:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZSLB || type == TransactionRecord::MNReward) {
